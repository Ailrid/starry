...
export async function handler(req: Request, res: Response): Promise<any> {
  ...
  if (source === 'local') {
    ...
    const response = {
      data: {
        id: id,
        url: `/api/song/data?id=${id}&md5=${rawResponse.data.md5 || ''}`,
        md5: rawResponse.data.md5,
        size: rawResponse.data.size,
        br: rawResponse.data.br,
        level: rawResponse.data.level
      },
      code: 200
    } as SongUrlResponse
    // 设置一个全局的url
    urlMap.set(id, rawResponse.data.url)
